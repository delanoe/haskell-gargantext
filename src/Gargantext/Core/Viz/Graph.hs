    ...
    insertChildren (Node { node_label, .. }) = Node { node_children = children', .. }
      where
        ...
        children' = case (lookup (NgramsTerm node_label) listNgrams) of
          Nothing  -> Nothing
          Just (NgramsRepoElement { _nre_children }) -> Just $ unNgramsTerm <$> mSetToList _nre_children
